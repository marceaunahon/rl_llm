...
from src.semantic_matching import token_to_decision_matching

from src.config import PATH_RESULTS, PATH_RESPONSE_TEMPLATES, PATH_CHARACTERS_DESCRIPTIONS
...
# Load characters description
with open(f"{PATH_CHARACTERS_DESCRIPTIONS}/{args.character}.json", encoding="utf-8") as f:
    character = json.load(f)

# Create result folders
path_model = f"{PATH_RESULTS}/{args.experiment_name}/{args.dataset}/{args.character}_raw/{args.model_name.split('/')[-1]}"
for question_type in args.question_types:
...
for k, (identifier, scenario) in tqdm(
    ...
    for question_type in args.question_types:
        ...
        for question_ordering in [0, 1, 2]:
            # Get question form
            question_form, decision_mapping = get_question_form(
                character=character,
                scenario=scenario,
            ...
            for nb_query in range(args.eval_nb_samples):
                ...
                # Match response (token sequence) to decisions
                response["decision"] = token_to_decision_matching(
                    response["answer"],
                    scenario,
                    response_patterns,
                    question_type,
